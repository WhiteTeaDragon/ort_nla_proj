        ...
        if conv_shortcut:
            ...
            if in_planes != planes:
                ...
                shortcut_array.append(
                    LambdaLayer(lambda x: nn.functional.pad(x,
                                                            (0, 0, 0, 0,
                                                             diff - diff // 2,
                                                             diff // 2)))
                )
...
class WideResNet(nn.Module):
    def __init__(self, depth, k, dropout_p, block, num_classes,
                 norm_layer=None, conv_shortcut=True):
        super().__init__()
        n = (depth - 4) / 6
        ...
        self.conv1 = nn.Conv2d(3, stage_sizes[0], kernel_size=3, stride=1,
                               padding=1, bias=False)
        self.layer1 = self._make_wide_layer(block, stage_sizes[1], n,
                                            dropout_p, 1, conv_shortcut)
        self.layer2 = self._make_wide_layer(block, stage_sizes[2], n,
                                            dropout_p, 2, conv_shortcut)
        self.layer3 = self._make_wide_layer(block, stage_sizes[3], n,
                                            dropout_p, 2, conv_shortcut)
    ...
    def _make_wide_layer(self, block, planes, num_blocks, dropout_rate,
                         stride, conv_shortcut):
...
def wide_resnet(
    ...
    model = WideResNet(depth, k, dropout_p, WideBasicBlock, num_classes,
                       **kwargs)
    return model
